...
\subsubsubsection{Back-propagation algorithm}
...
%
%\begin{align*}
%G(z) &= \frac{1}{1+e^{-(\theta_{g}z+b_{g})}} = \hat{x} = \sigma(\theta_{g}z+b_{g}) \\
%D(\hat{x}) &= \frac{1}{1+e^{-(\theta_{d}\hat{x}+b_{d})}} = \sigma(\theta_{d}\hat{x}+b_{d}) \\
%D({x}) &= \frac{1}{1+e^{-(\theta_{d}{x}+b_{d})}} = \sigma(\theta_{d}{x}+b_{d})
%\end{align*}
%
%Gradients, 
%\begin{align*}
%& \nabla_{\theta_{d}}\big(\frac{1}{m} \sum\limits^{m}_{i} \log D(x^{(i)})+ \log (1-D(G(z^{(i)}))\big)  \\
%& = \frac{1}{D(x)} D'(x) x - \frac{1}{1-D(\hat{x})}  D'(\hat{x}) \hat{x} \\
%& = \frac{1}{D(x)} D(x) (1-D(x)) x - \frac{1}{1-D(\hat{x})} D(\hat{x}) (1-D(\hat{x})) \hat{x} \\
%& = (1-D(x)) x - D(\hat{x}) \hat{x}
%\end{align*}
